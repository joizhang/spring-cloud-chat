...
import com.joizhang.chat.gateway.config.SpringDocConfiguration;
...
public class SwaggerBasicGatewayFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (hasAuth(exchange)) {
            ...
            response.getHeaders().add(HttpHeaders.WWW_AUTHENTICATE, "Basic Realm=\"chat\"");
    ...
    private boolean hasAuth(ServerWebExchange exchange) {
        ...
        String encodeToString = Base64Utils.encodeToString(
