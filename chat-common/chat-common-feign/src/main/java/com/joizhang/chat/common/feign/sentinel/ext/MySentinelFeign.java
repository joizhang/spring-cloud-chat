package com.joizhang.chat.common.feign.sentinel.ext;
        ...
        public Feign build() {
            super.invocationHandlerFactory(new InvocationHandlerFactory() {
                ...
                public InvocationHandler create(Target target, Map<Method, MethodHandler> dispatch) {
                    ...
                    FeignClient feignClient = AnnotationUtils.findAnnotation(
                    ...
                    if (void.class != fallback) {
                        fallbackInstance = getFromContext(
                                beanName, "fallback", fallback, target.type());
                        return new MySentinelInvocationHandler(
                                target, dispatch, new FallbackFactory.Default<>(fallbackInstance));
                    }

                    if (void.class != fallbackFactory) {
                        fallbackFactoryInstance = (FallbackFactory<?>) getFromContext(
                                beanName, "fallbackFactory", fallbackFactory, FallbackFactory.class);
                        return new MySentinelInvocationHandler(
                ...
                private Object getFromContext(String name,
                                              String type, Class<?> fallbackType,
                                              Class<?> targetType) {
